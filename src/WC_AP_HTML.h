 ...
 */
const char AP_HTTP_HEAD[] PROGMEM            = "<!DOCTYPE html><html lang=\"pt-BR\"><head><meta charset=\"utf-8\"><meta name=\"viewport\" content=\"width=device-width, initial-scale=1, user-scalable=no\"/><title>{v}</title>";
...
const char AP_HTTP_PORTAL_OPTIONS[] PROGMEM  = "<div class=\"msgbox\">Conecte este dispositivo a uma rede WiFi. Selecione a op&ccedil;&atilde;o para encontrar uma rede.</div><form action=\"/wifi\" method=\"get\"><button>Configurar WiFi (Busca Autom&aacute;tica)</button></form><br/><form action=\"/0wifi\" method=\"get\"><button style=\"background-color:#bbbbbb;\">Configurar WiFi (Manual)</button></form><!--<br/><form action=\"/i\" method=\"get\"><button>Info</button></form><br/></form>-->";
...
const char AP_HTTP_FORM_START[] PROGMEM      ="<form method=\"post\" action=\"wifisave\"><label>Nome da Rede WiFi (SSID):</label><input id=\"s\" name=\"s\" length=32 placeholder=\"Exemplo: MinhaRede_2024\"><label>Senha do WiFi:</label><input id=\"p\" name=\"p\" length=64 type=\"password\" placeholder=\"Senha123\"><br/>";
...
const char AP_HTTP_FORM_END[] PROGMEM        = "<br/><button type='submit'>Salvar e Conectar</button></form>";
/** HTML snippet to recan for networks */
const char AP_HTTP_SCAN_LINK[] PROGMEM       = "<br/><div class=\"c\"><a href=\"/wifi\">Buscar novamente</a></div>";
...
*/
const char AP_HTTP_SAVED[] PROGMEM           = "<div>Credenciais salvas.<br />Tentando conectar &agrave; rede WiFi. Aguarde.... <br /><script type=\"text/javascript\">function doPing(){attempt_count>20&&(window.clearInterval(myPinger),document.getElementById(\"conn_ok\").style.display=\"block\");var t=new XMLHttpRequest;t.onload=function(){console.log(this.responseText),document.getElementById(\"conn_fail\").style.display=\"block\",window.clearInterval(myPinger)},t.ontimeout=function(t){console.log(\"Timeout Counter is: \"+timeout_count++)},t.open(\"GET\",\"/foo\"),t.timeout=1e3,t.send(null),console.log(\"Ping counter is: \"+attempt_count++)}attempt_count=0,timeout_count=0;var myPinger=window.setInterval(doPing,3e3);</script><div class=\"cok\" id=\"conn_ok\">Conectado a {ap}!<br />Voc&ecirc; pode fechar esta janela. </div><div class=\"cfail\" id=\"conn_fail\">Falha ao conectar em {ap}!<br /><a href=\"/\">Clique aqui</a> para tentar novamente.</div></div>";
/** End of the HTML page */
const char AP_HTTP_END[] PROGMEM             = "</div></body></html>";
/** HTML snippet for our custom parameters portal form */
const char AP_HTTP_PORTAL_PARAM_OPTIONS[] PROGMEM  = "<form action=\"/params\" method=\"get\"><button>Configurar Par&acirc;metros</button></form><br/><form action=\"/i\" method=\"get\"><button>Informa&ccedil;&otilde;es</button></form><br/>";
